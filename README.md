...
This project aimes to compare the established bossdevice real-time setup with a Neurosimo phase estimation implementation in a dual closed-loop experiment.

## Directories
The `code` folder hosts all scripts relevant for running the actual experiment. Conversly, `data analysis` hosts the matlab and python code to calculate and plot the statistical results.
It uses `data` where all collected data as well as the random phases and filter coefficients are.
...
## Running the dual closed loop experiment
1. Ensure that the `Lava` folder exists on both the Control PC and Neurosimo PC. 
