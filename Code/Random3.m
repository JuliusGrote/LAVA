...
savedData.phases = [];
savedData_R3.singlephases = [];
savedData_R3.pairphases = [];
savedData_R3.MEPs.single = [];
savedData_R3.MEPs.paired = [];
savedData_R3.facs = [];
n_trials_paired = 1;
MEPs_P=[]
...
for i=1:length(All_phaseID)
        ...
        if New_All_phaseID(i)==1
            ...
            savedData_R3.pairphases = [savedData_R3.pairphases phase_choice];
            ...
            while mep_bug == false
                if(bd.triggers_remaining == 0)
                    temp = [];
                    trial_index = length(record.emg_R3)+1;
                    pause(EMG_pause)
                    try
                        record.emg_R3(trial_index).signal = bd.mep(sc.mep_channel, 100, 100)*5;
                    catch
                        disp('MEP missed. Retrying...')
                        continue
                    end
                    record.emg_R3(trial_index).signal = record.emg_R3(trial_index).signal - mean(record.emg_R3(trial_index).signal(sc.baselineTimeWindow(1) <= sc.emgTimeAxis & sc.emgTimeAxis <= sc.baselineTimeWindow(2)));
                    record.emg_R3(trial_index).type = 1;    % 1 for single pulse
                    temp = extractMEP(record.emg_R3(trial_index).signal,sc.emgTimeAxis,sc.mepTimeWindow,sc.baselineTimeWindow);
        ...
        elseif New_All_phaseID(i)==2
            ...
            while mep_bug == false
                if(bd.triggers_remaining == 0)
                    ...
                    record.emg_R3(trial_index).signal = record.emg_R3(trial_index).signal - mean(record.emg_R3(trial_index).signal(sc.baselineTimeWindow(1) <= sc.emgTimeAxis & sc.emgTimeAxis <= sc.baselineTimeWindow(2)));
                    record.emg_R3(trial_index).type = 2;    % 2 for paired pulse
                    temp = extractMEP(record.emg_R3(trial_index).signal,sc.emgTimeAxis,sc.mepTimeWindow,sc.baselineTimeWindow);
...
savedData_R3.All_phaseID=New_All_phaseID;
savedData_R3.All_pusle=New_All_pusle;
savedData_R3.Random_index=Random_index;

savedData_R3.MEPs.single = MEP_S;
savedData_R3.MEPs.paired = MEPs_P;   
savedData_R3.facs = mean(movmean(MEPs_P,3))/mean(movmean(MEP_S,3));

    
    
  
disp('Done')
savedData_Random3=savedData_R3;
saved_dataRandom3 = [folderpath,record.subject,'_',num2str(record.session),'_savedData_Random3','.mat']
save(saved_dataRandom3,'savedData_R3')