...
savedData_O2.phases = [];
savedData_O2.singlephases = [];
savedData_O2.pairphases = [];
savedData_O2.MEPs.single = [];
savedData_O2.MEPs.paired = [];
savedData_O2.facs = [];
n_trials_paired = 1;
MEPs_P=[]
...
for i=1:length(All_phaseID)
        ...
        if New_All_phaseID(i)==1
            ...
            savedData_O2.pairphases = [savedData_O2.pairphases phase_choice];
            ...
            while mep_bug == false
                if(bd.triggers_remaining == 0)
                    temp = [];
                    trial_index = length(record.emg_O2)+1;
                    pause(EMG_pause)
                    try
                        record.emg_O2(trial_index).signal = bd.mep(sc.mep_channel, 100, 100)*5;
                    catch
                        disp('MEP missed. Retrying...')
                        continue
                    end
                    record.emg_O2(trial_index).signal = record.emg_O2(trial_index).signal - mean(record.emg_O2(trial_index).signal(sc.baselineTimeWindow(1) <= sc.emgTimeAxis & sc.emgTimeAxis <= sc.baselineTimeWindow(2)));
                    record.emg_O2(trial_index).type = 1;    % 1 for single pulse
                    temp = extractMEP(record.emg_O2(trial_index).signal,sc.emgTimeAxis,sc.mepTimeWindow,sc.baselineTimeWindow);
        ...
        elseif New_All_phaseID(i)==2
            ...
            while mep_bug == false
                if(bd.triggers_remaining == 0)
                    ...
                    record.emg_O2(trial_index).signal = record.emg_O2(trial_index).signal - mean(record.emg_O2(trial_index).signal(sc.baselineTimeWindow(1) <= sc.emgTimeAxis & sc.emgTimeAxis <= sc.baselineTimeWindow(2)));
                    record.emg_O2(trial_index).type = 2;    % 2 for paired pulse
                    temp = extractMEP(record.emg_O2(trial_index).signal,sc.emgTimeAxis,sc.mepTimeWindow,sc.baselineTimeWindow);
...
savedData_O2.All_phaseID=New_All_phaseID;
savedData_O2.All_pusle=New_All_pusle;
savedData_O2.Random_index=Random_index;

savedData_O2.MEPs.single = MEP_S;
savedData_O2.MEPs.paired = MEPs_P;   
savedData_O2.facs = mean(movmean(MEPs_P,3))/mean(movmean(MEP_S,3));
    
  
disp('Done')
savedData_optimized2=savedData_O2;
saved_dataoptimized2 = [folderpath,record.subject,'_',num2str(record.session),'_savedData_optimized2','.mat']
save(saved_dataoptimized2,'savedData_O2')
