...
savedData_R4.phases = [];
savedData_R4.singlephases = [];
savedData_R4.pairphases = [];
savedData_R4.MEPs.single = [];
savedData_R4.MEPs.paired = [];
savedData_R4.facs = [];
n_trials_paired = 1;
MEPs_P=[]
...
for i=1:length(All_phaseID)
        ...
        if New_All_phaseID(i)==1
            ...
            savedData_R4.pairphases = [savedData_R4.pairphases phase_choice];
            ...
            while mep_bug == false
                if(bd.triggers_remaining == 0)
                    temp = [];
                    trial_index = length(record.emg)+1;
                    pause(EMG_pause)
                    try
                        record.emg_R4(trial_index).signal = bd.mep(sc.mep_channel, 100, 100)*5;
                    catch
                        disp('MEP missed. Retrying...')
                        continue
                    end
                    record.emg_R4(trial_index).signal = record.emg_R4(trial_index).signal - mean(record.emg_R4(trial_index).signal(sc.baselineTimeWindow(1) <= sc.emgTimeAxis & sc.emgTimeAxis <= sc.baselineTimeWindow(2)));
                    record.emg_R4(trial_index).type = 1;    % 1 for single pulse
                    temp = extractMEP(record.emg_R4(trial_index).signal,sc.emgTimeAxis,sc.mepTimeWindow,sc.baselineTimeWindow);
        ...
        elseif New_All_phaseID(i)==2
            ...
            while mep_bug == false
                if(bd.triggers_remaining == 0)
                    temp = [];
                    trial_index = length(record.emg_R4)+1;
                    pause(EMG_pause)
                    try
                        record.emg_R4(trial_index).signal = bd.mep(sc.mep_channel, 100, 100)*5;
                    catch
                    ...
                    record.emg_R4(trial_index).signal = record.emg_R4(trial_index).signal - mean(record.emg_R4(trial_index).signal(sc.baselineTimeWindow(1) <= sc.emgTimeAxis & sc.emgTimeAxis <= sc.baselineTimeWindow(2)));
                    record.emg(trial_index).type = 2;    % 2 for paired pulse
                    temp = extractMEP(record.emg_R4(trial_index).signal,sc.emgTimeAxis,sc.mepTimeWindow,sc.baselineTimeWindow);
...
savedData_R4.All_phaseID=New_All_phaseID;
savedData_R4.All_pusle=New_All_pusle;
savedData_R4.Random_index=Random_index;

savedData_R4.MEPs.single = MEP_S;
savedData_R4.MEPs.paired = MEPs_P;   
savedData_R4.facs = mean(movmean(MEPs_P,3))/mean(movmean(MEP_S,3));

    
    
  
disp('Done')
savedData_Random4=savedData_R4;
saved_dataRandom4 = [folderpath,record.subject,'_',num2str(record.session),'_savedData_Random4','.mat']
save(saved_dataRandom4,'savedData_R4')